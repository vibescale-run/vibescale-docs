...
<Frame caption="Relatório de segurança do VibeScale — grade, score, achados por categoria e ações de remediação">
  <img
    ...
    alt="Relatório de segurança: grade A, score 98/100"
  />
</Frame>
...
O relatório agora inclui um bloco de **Resumo Executivo** que apresenta uma narrativa completa e priorizada do estado de segurança do seu app. Este resumo fica destacado no topo da página, acima da grade de detalhes, para servir como uma introdução editorial clara.

Se o resumo estiver disponível, ele será exibido em texto corrido. Caso contrário, um placeholder animado indica que o conteúdo está sendo carregado.

Este recurso é ativado por uma configuração específica e pode ser visualizado por usuários com acesso autorizado.

## Distribuição por Track

A visualização da distribuição por categoria (Radar Chart) foi ajustada para aparecer sempre que houver dados suficientes, sem ser substituída pelo Resumo Executivo. Isso facilita a análise visual da severidade e distribuição dos achados por categoria.
