...
Na seção de resumo do relatório, a distribuição dos achados por "Track" (contexto de execução) é exibida como um gráfico de rosca acompanhado de uma legenda detalhada. Essa visualização substitui o gráfico radar anterior, que raramente era exibido devido à baixa diversidade de categorias em análises reais.

O gráfico de rosca mostra a quantidade de achados em cada categoria de Track, usando cores distintas para facilitar a identificação. A legenda abaixo do gráfico apresenta o nome da categoria e o número exato de achados correspondentes.

Essa mudança melhora a visualização e compreensão da distribuição dos achados, especialmente quando há poucas categorias predominantes, como execução e dependência.
