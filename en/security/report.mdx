...
description: "How to interpret the grade, finding categories, and severity levels — and how to act on each identified issue."
icon: "file-shield"
---
...
- **Summary** — a sentence reflecting the overall status:
  - *"X custom items require attention"*
...
Click **AI Agent Remediation Prompt** to copy a ready-to-use remediation prompt to paste into your preferred code assistant — Copilot, Cursor, Bolt, or similar. The prompt already includes the finding context and fix instructions.
...
<Warning>
  Re-Check will not be enabled while any Critical or High findings remain pending — regardless of category. Resolve or dismiss each to unlock this option.
</Warning>

## Distribution by Track

In the report summary section, the distribution of findings by "Track" (execution context) is shown as a donut chart accompanied by a detailed legend. This visualization replaces the previous radar chart, which was rarely displayed due to low category diversity in real analyses.

The donut chart shows the number of findings in each Track category, using distinct colors for easy identification. The legend below the chart lists the category name and the exact number of corresponding findings.

This change improves visualization and understanding of the findings distribution, especially when there are few predominant categories, such as execution and dependency.
